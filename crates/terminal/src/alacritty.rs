...
use crate::{
    ...
    Range, RenderableCells, Scroll, Search, Selection, SelectionRange, SelectionSide, SelectionType,
...
#[derive(Clone)]
pub(super) struct ZedListener(UnboundedSender<TerminalBackendEvent>);
...
pub(super) fn pty_term_config(scrolling_history: usize) -> AlacrittyTermConfig {
    Config {
        scrolling_history,
        default_cursor_style: alacritty_cursor_style(CursorShape::Block),
        //TODO: vi mode cursor shape and osc52
        ..Config::default()
    }
}
...
pub(super) fn new_term(
    ...
    let term = Term::new(config.clone(), &bounds, ZedListener(events_tx));
...
impl EventListener for ZedListener {
    fn send_event(&self, event: AlacTermEvent) {
        self.0.unbounded_send(event.into()).ok();
    }
}
...
pub(super) fn make_content(term: &Term<ZedListener>, last_content: &Content) -> Content {
    ...
    // 视口最下面那一行是否被占用：光标已经在/越过它，或该行有非空格字符。
    // 供渲染层判断要不要把网格「底部锚定」（见 zed 上游同名实现）。
